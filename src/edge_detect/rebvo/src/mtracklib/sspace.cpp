...
/*
  ...
  This class implements the space of scales (Lindenberg 94.98 and others ...) to Lowe et. to the. (SIFT) 
  Each octaba under-employs the previous octave by two, to save calculation time.
  However, if features are to be searched for in s_n scales, s_n + 3 scales per octave should be calculated 
  since s_n + 2 DoG must be available to search for maximum in scale space
*/
...
void sspace::build(Image<DetectorImgType> &data){
    ...
    filter0.smooth(data,img[0]);    
    filter1.smooth(data,img[1]);    
