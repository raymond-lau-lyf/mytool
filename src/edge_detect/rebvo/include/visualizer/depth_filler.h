...
public:
    ...
    TooN::Vector <3> getImg3DPos(float x,float y,cam_model &cam){
        ...
        int xf=std::min<float>(std::max<float>(floor(x_histo),0.0),size.w-1.0);
        int xc=std::min<float>(std::max<float>(ceil(x_histo),0.0),size.w-1.0);

        float y_histo=y/(float)bl_size.h-0.5;
        int yf=std::min<float>(std::max<float>(floor(y_histo),0.0),size.h-1.0);
        int yc=std::min<float>(std::max<float>(ceil(y_histo),0.0),size.h-1.0);
